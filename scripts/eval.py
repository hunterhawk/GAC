...
tf.app.flags.DEFINE_string('eval_dir', '/tmp/gac-eval',
                           """Directory where to write event logs.""")
tf.app.flags.DEFINE_string('eval_file', 'tfrecords/test.tfrecords',
                           """Evaluate performance on test set.""")
tf.app.flags.DEFINE_integer('num_examples', 200,
                            """Number of examples to run.""")
tf.app.flags.DEFINE_integer('batch_size', BATCH_SIZE,
                            """Batch size to use.""")
tf.app.flags.DEFINE_string('checkpoint_dir', '/tmp/gac-train',
...
def eval_once(saver, summary_writer, top_k_op, summary_op):
    ...
    with tf.Session() as sess:
        ...
        # Create a coordinator for threads
...
def evaluate():
    """Eval gac for a number of steps."""
    with tf.Graph().as_default() as g:
        # Get test images and test labels.
        eval_file = FLAGS.eval_file
        images, labels = gac.tf.preprocessing.inputs(
            eval_file, FLAGS.batch_size)
